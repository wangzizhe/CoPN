model Exclusion
  ...
  PNlib.Components.TE T1a(nOut = 1, nIn = 1, event = {1, 6, 9})  annotation(
    Placement(transformation(origin = {-70, 36}, extent = {{-10, -10}, {10, 10}})));
  PNlib.Components.TE T1d(nIn = 1, event = {3, 10})  annotation(
    Placement(transformation(origin = {62, 36}, extent = {{-10, -10}, {10, 10}})));
  PNlib.Components.TE T2a(nOut = 1, nIn = 1, event = {2, 5})  annotation(
    Placement(transformation(origin = {-70, -46}, extent = {{-10, -10}, {10, 10}})));
  PNlib.Components.TE T2d(nIn = 1, event = {8})  annotation(
...
equation
  ...
  connect(IA1.outTransition, T1a.inPlaces[1]) annotation(
    Line(points = {{-8, 8}, {-74, 8}, {-74, 36}}));
  connect(P1.outTransition[2], IA2.inPlace) annotation(
    Line(points = {{4, 36}, {-48, 36}, {-48, -7}}));
  connect(IA2.outTransition, T2a.inPlaces[1]) annotation(
